    ...
    <div class="space-y-4 max-h-80 overflow-y-auto mb-6">
      ...
      >
        ...
        <div class="ml-4 flex-1">
          <p class="text-xl font-bold text-gray-800 mb-1">{{ child.name }}</p>
          <p class="text-base text-gray-600">
            {{ calculateAge(child.birthDate) }}세
          </p>
          <p v-if="child.interests && child.interests.length > 0" class="text-sm text-gray-500 mt-1">
            관심사: {{ child.interests.slice(0, 2).join(', ') }}{{ child.interests.length > 2 ? ' 외' : '' }}
          </p>
        </div>
