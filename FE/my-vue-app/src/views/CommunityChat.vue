...
const loadMessages = async () => {
  ...
  try {
    ...
    log("메시지 히스토리 로드 완료:", chatMessages.value.length, "개");
  } catch (error) {
    error("메시지 목록 로드 실패:", error);
  }
};
...
const loadCommunityData = async () => {
  ...
  try {
    ...
    log(`커뮤니티 ${communityId} 구독 완료`);
  } catch (error) {
    error("채팅방 로드 실패:", error);
