...
async function fetchMonthlyDiaries() {
  try {
    ...
    if (!childId) {
      warn('childId가 없어서 다이어리 조회를 건너뜁니다.');
      diaries.value = [];
      return;
    }
    ...
    log('요청 파라미터:', { childId, year, month });
    const response = await childService.getMonthlyDiaries(childId, year, month);
    log('응답 결과:', response);
    log('response 타입:', typeof response, 'isArray:', Array.isArray(response));
    ...
    log('최종 변환된 diaries:', diaries.value);
  ...
  dayCellDidMount: (info) => {
    ...
    const checkAndRender = () => {
      ...
      if (d && d.imageUrl) {
        ...
        log(`${key} 이미지 렌더링 완료 (${attempts}번째 시도)`);
